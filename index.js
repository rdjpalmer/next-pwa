...
module.exports = (nextConfig = {}) => ({
  ...
  webpack(config, options) {
    ...
    if (typeof register !== "boolean" && register !== "custom") {
      console.error(
        '> [PWA] `register` configuration must be one of `true`, `false` or `"custom"`'
      );
    }
    ...
    if (!options.isServer) {
      ...
      if (register === "custom") {
        console.log(
          `> [PWA] Using custom service worker registeration. Please call \`window.workbox.register()\` in componentDidMount callback or useEffect hook`
        );
      }
      if (register !== "custom" && register) {
        console.log(
